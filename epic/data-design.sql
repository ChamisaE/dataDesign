...
-- creating tables
create table author (
	authorId BINARY(2) not null, -- this will create a 32 digit id
...
create table searchFilter (
	filterId BINARY(2) not null, -- PK, this will create a 16 digit filter id
...
create table article (
	articleAuthorId BINARY(2) not null, -- keeping consistent with all other ID lengths/types
	articleId BINARY(16) not null, -- ID is binary, this allows lots of numbers : a nearly infinite # articles can be written
	articleFilterId BINARY(2) not null, -- ID is binary, linking it to filterId in searchFilter table
